...
class RRInputExample(object):
    def __init__(self, id, context, question, node_label, edge_label, label, masked=False):
        self.id = id
        self.context = context
        self.question = question
    ...
    def _create_examples(self, records, meta_records):
        ...
        for (i, (record, meta_record)) in enumerate(zip(records, meta_records)):
            ...
            for (j, question) in enumerate(record["questions"]):
                ...
                id = question["id"]
                label = question["label"]
                masked = question.get("masked", False)
                question = question["text"]
                meta_data = meta_record["questions"]["Q"+str(j+1)]
                ...
                examples.append(RRInputExample(id, context, question, node_label, edge_label, label, masked))
...
def convert_examples_to_features_RR(examples,
    ...
    max_size = 0
    if filter_mask:
        examples = [example for example in examples if not example.masked]
    for (ex_index, example) in enumerate(examples):
        ...
        if example.masked:
            node_label = [x - 2 if x in [0, 1] else x for x in node_label]
        node_label = node_label + [-100] * (max_node_length - len(node_label))

        edge_label = example.edge_label
        if example.masked:
            edge_label = [x - 2 if x in [0, 1] else x for x in edge_label]
