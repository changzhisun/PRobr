...
def get_fact_rule_identifiers(data_dir, natlang_metadata, eval_split):
    test_file = os.path.join(data_dir, "%s.jsonl" % eval_split)
    meta_test_file = os.path.join(data_dir, "meta-%s.jsonl" % eval_split)
...
def process(param_list):
    edge_logit, node_pred, fact_rule_identifier = param_list
    ...
    edges = inference.solve_LP(edge_logit, fact_rule_identifier, node_pred)
    #edges = inference.solve_LP_unconstrained(edge_logit)
    return edges

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", default=None, type=str, required=True)
    parser.add_argument("--eval_split", default=None, type=str, required=True)
    ...
    parser.add_argument("--n_cpu", default=20, type=int)
    ...
    fact_rule_identifiers = get_fact_rule_identifiers(args.data_dir, args.natlang_metadata, args.eval_split)
    ...
    pool = ProcessPool(min(args.n_cpu, n_cpu))
    results = pool.map(process, zip(edge_logits, node_preds, fact_rule_identifiers))
    pool.close()
    pool.join()
